...
static void print_usage(char **argv)
{
	printf("Usage:\n"
			...
			"       service run %s -args \"speakerPWM <freq, filename.pcm>\"\n"
			"       service run %s -args \"speaker1bit <filename.csv>\"\n",
...
static int proc_args(int argc, char **argv)
{
	...
	//timer_test_int(time)
		...
		if (argc != 4) {
			printf("timer: wrong number of arguments for timer_test_speaker()\n");
			return 1;
		}
		...
		return timer_test_speaker(freq, argv[3]);
		...
		return disable_speaker();
	} else if(strncmp(argv[1], "speaker1bit", strlen("speaker1bit")) == 0) {
		if (argc != 3) {
			printf("timer: wrong number of arguments for speaker 1-bit mode\n");
			return 1;
		}
