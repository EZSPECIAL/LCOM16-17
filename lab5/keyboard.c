...
int kbd_check_esc() {
	...
	int keyboard_hook = kbd_subscribe_int();
...
int kbd_controller_choice() {
	...
	printf("Choose test_controller operation with keyboard (no numpad):\n");
	printf("   1 - Use VideoModePtr to find available video modes\n");
	printf("   2 - Use Reserved Area of Function 00h struct to find available video modes\n");
	printf("   3 - Use both methods, useful for testing on hardware\n");

	//Handle interrupts until user chooses
	while(kbd_code != BREAK_1 && kbd_code != BREAK_2 && kbd_code != BREAK_3) {
		if ((dstatus = driver_receive(ANY, &msg, &ipc_status)) != 0) { //Receives any messages
			printf("driver_receive failed with: %d", dstatus);
			continue;
		}
